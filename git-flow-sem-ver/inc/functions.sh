...
LAST_TAG=$(git ls-remote --tags -q --sort="v:refname"| grep -o "$SEMVER_REGEX" | tail -n1)
...
function __get_next_version
{
    ...
    if [ ${#SEMVER[@]} -ne 3 ]; then
      echo "ERROR${COLOR_RED}Semantic Version number doesn't have correct format X.Y.Z (major.minor.patch)(${SEMVER_TAG})(${SEMVER}).${COLOR_DEFAULT}"
        exit 78
    fi
    ...
    echo $(__get_version_prefix)${SEMVER[0]}.${SEMVER[1]}.${SEMVER[2]}
}
